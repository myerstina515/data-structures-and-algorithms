...
const templateWithJQuery = () => {
  starWarsPeople.forEach( starPeople => {
    var template = $('section').html();
    var noIdea = $(`<section>${template}</section>`);
    
    noIdea.find('h2').text(starPeople.name);
    noIdea.find('h3').text(starPeople.height);
    noIdea.find('p').text(starPeople.eye_color);
...
// index of second space, then slice.
